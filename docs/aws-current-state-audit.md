...
| No environment tags on any resource | **Environment-management gap** | Expected, since environment separation doesn't exist yet — this is exactly what `docs/environment-separation-plan.md` §4's naming/tagging standard is for, applied going forward. |
...
| OAuth authorization-code flow configured with a live callback URL, unclear current usage | **Unresolved / needs a decision** | Not necessarily a defect, but its purpose and whether it's actively used should be confirmed before Development/Staging pools are provisioned, so the new pools' OAuth configuration (if any) is deliberate, not copied blindly. |
