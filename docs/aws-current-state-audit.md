...
| Lambda runtime `nodejs24.x`, `arm64`, 128 MB, 3s timeout | **Acceptable for Production as observed here; proven insufficient in Development** | Modern runtime, cost-efficient architecture/memory; no throttling/timeout evidence in the *Production* metadata reviewed for this 2026-08-08 audit. **Update:** the identical `128 MB / 3 s` sizing on the CDK-managed *Development* sync Lambdas later caused `interval-dev-sync-push` to time out at exactly 3000 ms on every invocation (2026-08 incident). Development/Staging sync Lambdas were raised to `256 MB / 15 s` — see `docs/cdk-infrastructure.md`'s "Sync Lambda sizing incident (2026-08)". Whether Production's grandfathered Lambdas need the same change is a separate, founder-gated question. |
